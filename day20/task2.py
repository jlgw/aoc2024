...
for i in range(1, 21):
    for j in range(i + 1):
        if (i, j) == (0, 0):
            continue
        cheats.add(add_tuple(mult_tuple_scalar(up, j), mult_tuple_scalar(left, i - j)))
        cheats.add(
            add_tuple(mult_tuple_scalar(down, j), mult_tuple_scalar(left, i - j))
        )
        cheats.add(add_tuple(mult_tuple_scalar(up, j), mult_tuple_scalar(right, i - j)))
        cheats.add(
            add_tuple(mult_tuple_scalar(down, j), mult_tuple_scalar(right, i - j))
        )
