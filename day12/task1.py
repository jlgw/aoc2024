...
def calc_circumference(coordinate_set):
    sm = 0
    for s in coordinate_set:
        sm += 4 - neighbor_count(coordinate_set, s)
    return sm


def calc_price(coordinate_set):
    return len(coordinate_set) * calc_circumference(coordinate_set)
...
def neighbor_count(coordinate_set, coordinates):
    sm = 0
    if add_tuple(coordinates, up) in coordinate_set:
        sm += 1
    if add_tuple(coordinates, down) in coordinate_set:
        sm += 1
    if add_tuple(coordinates, left) in coordinate_set:
        sm += 1
    if add_tuple(coordinates, right) in coordinate_set:
        sm += 1
    return sm
