...
def move_keypad(current_value, next_value):
    ...
    elif current_position[0] == 0 and next_position[1] == 3:
...
def move_direction_pad(current_value, next_value):
    current_position = direction_pad[current_value]
    next_position = direction_pad[next_value]
    change = subtract_tuple(next_position, current_position)
    horizontal = -change[0] * "<" if change[0] < 0 else change[0] * ">"
    vertical = -change[1] * "^" if change[1] < 0 else change[1] * "v"
    if current_position[1] == 0 and next_position[0] == 0:
        out = vertical + horizontal
    elif current_position[0] == 0 and next_position[1] == 0:
        out = horizontal + vertical
...
def move_direction_pad_all(sequence):
    output = ""
    current = "A"
    for s in sequence:
        local_output = move_direction_pad(current, s)
        output += local_output + "A"
        current = s
    return output
...
for line in lines:
    ...
    seq2 = move_direction_pad_all(seq)
    seq3 = move_direction_pad_all(seq2)
    solutions.append(seq3)
